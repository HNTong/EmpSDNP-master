...
switch learner
    ...
    case 'BRR'
        ...
        predY = mdl.predict(validation(:,1:end-1));
    ...
    case 'SVR'
        ...
        predY = predict(mdl, validation(:,1:end-1));
    case {'pr','nbin','zip'}
        mdl = py.CountModelFun.count_model(py.numpy.array([newTrainXBal,newTrainYBal]), py.numpy.array([validation]), learner);
        predY = mdl{1}.double;
end
...
rand('seed',seed); % rand('seed', seed); % For reproduction
